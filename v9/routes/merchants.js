...
Merchant.findById(req.params.id).exec(function(err, thisMerchant){
...
router.post("/merchants/:id",isLoggedIn, function(req, res){
    ...
    var newMerchant = {name: name, image: image, description: desc, author: author};
    // add a merchant
    Merchant.create(newMerchant, function(err, created){
        if(err){
            console.log("error" + err);
